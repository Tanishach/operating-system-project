...
int total_time = 0, processes_size, queue_count;

int next_process_to_execute()
{
    int high_priority = 0, less_burst_time = 0, process_index = 0;

    for (int index = 0; index < processes_size; index++)
    {
...
void display_function()
{
    ...
    total_turnaround_time /= processes_size;

    for (int index = 0; index < processes_size; index++)
    {
        total_waiting_time += process[index].waiting;
    }
    total_waiting_time /= processes_size;
...
int main()
{	
	cout<<"\t\t..............PROJECT OF OPERATING SYSTEM PRIORITY SCHEDULING  AND SJF WITH AGING......................\n\n";
    cout << "number of processses you want to execute : ";
    cin >> processes_size;
    process = new processes[processes_size];
    cout<<"||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n";
    cout << "\nPLEASE GIVE INPUT IN THE FORM  => Arrival_Time      Burst_timeT\n\n\n";
